...
export function runEditQa({ motionPlan, narrationSpec, plan = {}, manifest = null }, { maxDullGapSec = 30, minCoverage = 0.5 } = {}) {
  ...
  // 2b. Visual material — the direct check the proxies above cannot stand in for
  // (2026-07-12 incident: a 47s cut with ZERO bound assets sailed through on
  // narration coverage + cut counts and rendered as one background fallback for the
  // whole runtime). A scene is "bare" when it carries no bound asset; a scene the
  // fallback path already degraded (assetFallback) is knowingly bare — warned, since
  // it is flagged dirty for re-render elsewhere. An ENTIRELY bare cut is blocked:
  // full-length background-fallback video is a dead render, not a style.
  const scenes = motionPlan?.scenes || []
  const bare = scenes.filter((s) => !(Array.isArray(s.assets) && s.assets.length > 0))
  if (scenes.length > 0 && bare.length === scenes.length) {
    blocking.push(`no scene carries any visual asset — the whole cut would render on the background fallback; bind imagery/captures (fetch-assets + bindAssetsToPlan) before ship`)
  } else {
    for (const s of bare) {
      warnings.push(
        s.assetFallback
          ? `scene "${s.beatId}" ships on the placeholder (missing: ${s.assetFallback.missing.join(', ')}) — re-render when assets land`
          : `scene "${s.beatId}" has no visual asset bound — background fallback only`,
      )
    }
  }

  // 2c. Grammar variety — one scene treatment repeated across the whole cut is a
  // slideshow, not a video (the second 2026-07-12 critique: "the same exact
  // slideshow formula every single thing"). The §19 scene library exists so beats
  // can LOOK different; a cut of ≥4 scenes all sharing one sceneType is flagged
  // loudly. Warning, not block — a uniform grammar must be a visible choice.
  if (scenes.length >= 4) {
    const types = new Set(scenes.map((s) => s.params?.sceneType ?? 'photo-caption'))
    if (types.size === 1) {
      warnings.push(`all ${scenes.length} scenes use the single grammar "${[...types][0]}" — a slideshow, not a video; vary params.sceneType across the §19 scene library (kinetic-open, mechanism, timeline, reveal, counter, chart, money-payoff, recap)`)
    }
  }
