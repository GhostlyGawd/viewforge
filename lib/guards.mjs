...
export const HARD_CONSTRAINTS = Object.freeze([
  ...
  {
    id: 'research-only-never-published',
    description:
      'Assets ingested for research (competitor pulls, yt-dlp material) are for analysis only. They may never appear in a published render — a rights and Content ID risk regardless of how good the shot is. Enforced at bind and assembly time, not by convention.',
    severity: 'block',
  },
])
...
export function checkHardConstraints(plan = {}) {
  ...
  // research-only-never-published: the assembly carries research-ingested material.
  if (plan.usesResearchOnlyAssets === true) {
    violations.push({ ...flag('research-only-never-published'), field: 'usesResearchOnlyAssets' })
  }
...
//   { videoId, cohort: "train"|"holdout", target: {metric, delta}, guards: [{metric, delta}],
//     simulated?:bool, provenance?: "youtube_api"|"manual"|"simulated",
//     evidenceClass?: "experiment"|"observational"|"comments", topicCluster?: string }
...
export const METRIC_PROVENANCE = Object.freeze(['youtube_api', 'manual', 'simulated'])
export const EVIDENCE_CLASSES = Object.freeze(['experiment', 'observational', 'comments'])
...
export const PROMOTION_DEFAULTS = Object.freeze({
  ...
  minDistinctTopicClusters: 2, // when clusters are logged, wins must span ≥ this many
})

/**
 * Evidence admissibility (Master Doc v2 §17/§23): only measurements that actually
 * came from the platform API may advance a rule. `manual` numbers are stored but
 * advance nothing; `simulated` is radioactive. Observations predating the
 * provenance field keep the old rule (admissible iff not simulated) so history
 * neither launders itself in nor gets thrown away.
 */
export function isAdmissibleEvidence(o) {
  if (!o || o.simulated) return false
  if (o.provenance === undefined) return true // legacy observation
  return o.provenance === 'youtube_api'
}
...
export function evaluatePromotion(observations = [], opts = {}) {
  ...
  const admissible = observations.filter(isAdmissibleEvidence)
  if (admissible.length < observations.length) {
    reasons.push(`ignored ${observations.length - admissible.length} inadmissible observation(s) — only real youtube_api measurements can promote (no simulated/manual data)`)
  }

  // Comments are direction-finding signal (§10) — they may move a strategy into
  // testing, but qualitative synthesis can never SATISFY the promotion bar (§23).
  const real = admissible.filter((o) => o.evidenceClass !== 'comments')
  if (real.length < admissible.length) {
    reasons.push(`ignored ${admissible.length - real.length} comments-class observation(s) for promotion — qualitative signal never validates a rule`)
  }
  ...
  const holdoutWinObs = real.filter((o) => o.cohort === 'holdout' && (o.target?.delta ?? 0) > 0)
  const holdoutWins = holdoutWinObs.length
  ...
  // Topic-cluster confound guard (v2 §10): when clusters are logged, the winning
  // evidence must span enough distinct topics that a topic effect can't be
  // mistaken for a style effect. Engages only when clusters are present at all —
  // legacy evidence without clusters is not retroactively blocked.
  let clustersOk = true
  const clustered = real.some((o) => o.topicCluster !== undefined && o.topicCluster !== null)
  if (clustered) {
    const winClusters = new Set(holdoutWinObs.filter((o) => o.topicCluster != null).map((o) => o.topicCluster))
    if (winClusters.size < cfg.minDistinctTopicClusters) {
      clustersOk = false
      reasons.push(`holdout wins span ${winClusters.size} topic cluster(s), need ≥${cfg.minDistinctTopicClusters} — a topic effect must not masquerade as a style effect`)
    }
  }
