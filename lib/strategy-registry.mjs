...
// ---------------------------------------------------------------------------
// Rule domains (Master Doc v2 §10): CTR is mostly title+thumbnail (packaging);
// retention/watch/satisfaction is mostly the video itself (content). A strategy's
// domain is WHERE ITS EVIDENCE LIVES, so it follows the target metric — and the
// analytics loop refuses to let one domain's metric write the other domain's rules.
// ---------------------------------------------------------------------------
export const RULE_DOMAINS = Object.freeze(['packaging', 'content'])

export const METRIC_DOMAINS = Object.freeze({
  CTR: 'packaging',
  AVD: 'content',
  AVP: 'content',
  retention_30s: 'content',
  retention_50pct: 'content',
  likeRatio: 'content',
  commentRate: 'content',
  subsPerView: 'content',
  returnViewerRate: 'content',
  sessionTime: 'content',
})

/** The domain a metric's evidence belongs to. */
export function domainOfMetric(metric) {
  const d = METRIC_DOMAINS[metric]
  if (!d) throw new Error(`domainOfMetric: unknown metric "${metric}"`)
  return d
}

/** A strategy's rule domain: the explicit field when present (validated consistent
 *  with its targetMetric), else derived from the metric. */
export function strategyDomain(s) {
  if (s?.domain) return s.domain
  return domainOfMetric(s?.targetMetric)
}
...
export function validateStrategy(s) {
  ...
  // Rule domain (v2 §10): must be a known domain and CONSISTENT with the target
  // metric — a "packaging" rule whose evidence metric lives in the content domain is
  // exactly the cross-domain confusion the doctrine forbids.
  if (s.domain !== undefined) {
    if (!RULE_DOMAINS.includes(s.domain)) {
      errors.push(`domain must be one of ${RULE_DOMAINS.join('|')}: got ${s.domain}`)
    } else if (s.targetMetric !== undefined && METRIC_DOMAINS[s.targetMetric] && METRIC_DOMAINS[s.targetMetric] !== s.domain) {
      errors.push(`domain "${s.domain}" contradicts targetMetric ${s.targetMetric} (a ${METRIC_DOMAINS[s.targetMetric]}-domain metric) — evidence cannot cross domains`)
    }
  }

  // Expiry (v2 §23): observational/comments-backed rules must not live forever
  // unvalidated. Optional positive integer, counted in published videos.
  if (s.expiresAfterVideos !== undefined && !(Number.isInteger(s.expiresAfterVideos) && s.expiresAfterVideos > 0)) {
    errors.push(`expiresAfterVideos must be a positive integer: got ${s.expiresAfterVideos}`)
  }
...
export function queryStrategies(strategies, filter = {}) {
  ...
  return strategies
    .filter((s) => {
      ...
      if (filter.domain && strategyDomain(s) !== filter.domain) return false
