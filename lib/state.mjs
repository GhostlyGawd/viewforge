...
/**
 * Set the channel's brand record (immutably) and log it. The caller is expected to
 * validate completeness first (see brand-brief.validateBrandRecord) — this stores
 * what it's given so state.mjs stays free of brand-specific schema.
 */
export function setBrand(root, slug, brand, now = defaultNow) {
  if (!brand || typeof brand !== 'object') throw new Error('setBrand: brand must be an object')
  return updateChannel(root, slug, { brand }, { event: 'brand.set', detail: { name: brand.name ?? null } }, now)
}

/**
 * Append a video to the channel (immutably). Assigns a stable id (vid-N) if absent,
 * requires a title, and stamps a created timestamp + initial stage. Returns the new
 * channel object.
 */
export function addVideo(root, slug, video, now = defaultNow) {
  if (!video || typeof video !== 'object' || !video.title) throw new Error('addVideo: video must have a title')
  const current = loadChannel(root, slug)
  const id = video.id || `vid-${current.videos.length + 1}`
  if (current.videos.some((v) => v.id === id)) throw new Error(`addVideo: duplicate video id ${id}`)
  const ts = now()
  const entry = { id, stage: video.stage || 'idea', createdUtc: ts, ...video }
  return updateChannel(root, slug, { videos: [...current.videos, entry] }, { event: 'video.added', detail: { id, title: video.title } }, now)
}
