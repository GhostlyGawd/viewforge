 ...
 * muddy-values amateur tell. This measures FLATNESS, not darkness (a mid-bright
 * flat frame fails too). A scene that DECLARES itself a minimal/dark beat
 * (params.dark → declaredDark) is exempt: caught running the guard on the real
 * c20 delivery, where the deliberate centered-text-on-black cold-open and stinger
 * scored 0.02 spread — legitimately sparse title cards, not muddy world scenes.
 * Declared intent, like empty-frame's exemption; a WORLD scene that renders flat
 * (the c19 Capitol, undeclared) is still caught.
 */
export function checkValueStructure(image, { sceneId = null, position = null, declaredDark = false, grid = VALUE_STRUCTURE } = {}) {
  const { blockLumaStd } = valueStructure(image, { grid })
  const findings = []
  if (!declaredDark && blockLumaStd < grid.minBlockLumaStd) {
    const where = position ? ` (${position})` : ''
