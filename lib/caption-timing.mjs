...
/**
 * Merge REAL forced-alignment output (whisperX over the full narration track) into a
 * captions object: each beat's `revealSec` is replaced with the aligned word times.
 * `alignedWords` is the flat, track-absolute list [{ word|text, start|startSec }…] in
 * spoken order. Rules, enforced not assumed:
 *  - the aligned word count must equal the captions' total word count, and each
 *    beat's words must match its aligned slice (normalized: case/punctuation
 *    stripped) — a mismatch names the beat and index, never silently drops;
 *  - times may spill into the inter-beat gap by up to `graceSec` (synth gaps are
 *    ~0.32s); such reveals clamp to the beat end, anything further is an error.
 * Returns { valid, errors, captions } — `captions` is a NEW object when valid.
 */
export function mergeAlignmentIntoCaptions(captions, alignedWords, { graceSec = 0.35 } = {}) {
  const errors = []
  const beats = captions?.beats
  if (!Array.isArray(beats) || beats.length === 0) return { valid: false, errors: ['captions has no beats'], captions: null }
  if (!Array.isArray(alignedWords) || alignedWords.length === 0) return { valid: false, errors: ['no aligned words'], captions: null }

  const totalWords = beats.reduce((a, b) => a + (b.words?.length ?? 0), 0)
  if (alignedWords.length !== totalWords) {
    return { valid: false, errors: [`aligned word count ${alignedWords.length} ≠ captions word count ${totalWords} — refuse to guess the mapping`], captions: null }
  }
  ...
  for (const b of beats) {
    const slice = alignedWords.slice(cursor, cursor + b.words.length)
    cursor += b.words.length
    b.words.forEach((w, i) => {
      const aw = slice[i]?.word ?? slice[i]?.text
      if (norm(w) !== norm(aw)) errors.push(`beat "${b.beatId}" word ${i}: caption "${w}" vs aligned "${aw}" — transcripts disagree`)
    })
    const na = normalizeAlignedWords(slice, { beatStartSec: b.startSec, durSec: b.durSec + graceSec })
    errors.push(...na.errors.map((e) => `beat "${b.beatId}": ${e}`))
    if (na.valid) {
      // clamp grace-window spill to just inside the beat (a caption may hold, never lie)
      const revealSec = na.words.map((x) => Math.min(x.tSec, Math.max(0, b.durSec - 0.001)))
      mergedBeats.push({ ...b, revealSec, revealSource: 'forced-alignment' })
    }
  }
  if (errors.length) return { valid: false, errors, captions: null }
  return { valid: true, errors: [], captions: { ...captions, beats: mergedBeats } }
}
