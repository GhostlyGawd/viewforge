...
| 6 | **motion** | Produce the visuals — **audio-first**: the timing solver resolves scene durations from the real narration (VO overruns bounce to script), then the parameterized timeline → Remotion-first motion graphics, per-scene render cache | `lib/motion-plan.mjs` + `lib/timing-solver.mjs` + `lib/render-cache.mjs` (+ Remotion) | **L2** (skill: `motion-graphics`) |
| 7 | **edit** | Assemble + tighten ("no dull moments"), master to −14 LUFS / −1 dBTP, final ship/block QA against hard constraints incl. research-only-never-published (manifest scan) | `lib/edit-qa.mjs` + `lib/audio-mix.mjs` | **L2** (skill: `edit-assemble`) |
